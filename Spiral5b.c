...
void PezInitialize()
{
    ...
    for (int i = 0; i < InstanceCount; i++) {
        ...
        m->Center.x = (rand() % 12) / 12.0f - 0.5f;
        m->Center.y = (rand() % 12) / 12.0f - 0.5f;
        m->Center.z = (rand() % 12) / 12.0f - 0.5f;
    }
...
void PezRender()
{
    ...
    for (int i = 0; i < InstanceCount; i++) {
        ...
        Matrix4 modelMatrix = M4Mul(M4MakeRotationZ(theta), M4MakeTranslation(m->Center));
        Matrix4 modelview = M4Mul(Scene.ViewMatrix, modelMatrix);
        Matrix3 normalMatrix = M4GetUpper3x3(modelview);
        float* pModel = (float*) &modelMatrix;
        float* pModelview = (float*) &modelview;
        float* pNormalMatrix = (float*) &normalMatrix;
